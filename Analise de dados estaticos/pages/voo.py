...
def calc_derivada(x, y):
    dados_result = {'Info': ['VelocidadeMax', 'Hxmax', 'Hymax', 'VM', 'Pontos', 'Duração', 'Classificação', 'Nome'],
...
def save(file, ndf, fig):
    ...
    with pd.ExcelWriter(file+'.xlsx') as writer:
        ...
        df_result.to_excel(writer, sheet_name='Resultados do Voo')
...
def update_graph(input_data, input_data2):
    ...
    calc_derivada(ndf['Tempo de Queima'], ndf['Força'])
    max_x = 0.0
    max_y = 0.0
